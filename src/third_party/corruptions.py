# -*- coding: utf-8 -*-
"""
This file contains the code for the ImageNet-C corruptions.
It has been adapted from the original code at: https://github.com/hendrycks/robustness
The adaptation includes:
    - Variable image size (not fixed to 256x256)
    - Remove FSGM
    - Change Motion blur and avoid using the Wand library
"""
...
TABULAR_ADDITIVE_SCALE = [0.02, 0.04, 0.06, 0.08, 0.1]
TABULAR_MULTIPLICATIVE_SCALE = [0.04, 0.08, 0.12, 0.16, 0.2]
...
def gaussian_noise(x, img_size: int, severity=1):
    if img_size == 64:
        ...
        raise ValueError(
            f"Gaussian noise not defined for image size {img_size}")
...
def impulse_noise(x, img_size: int, severity=1):
    if img_size == 64:
        ...
        raise ValueError(
            f"Impulse noise not defined for image size {img_size}")
...
def speckle_noise(x, img_size: int, severity=1):
    if img_size == 64:
        ...
        raise ValueError(
            f"Speckle noise not defined for image size {img_size}")
...
def gaussian_blur(x, img_size: int, severity=1):
    if img_size == 64:
        ...
        raise ValueError(
            f"Gaussian blur not defined for image size {img_size}")
...
def glass_blur(x, img_size: int, severity=1):
    ...
    x = np.uint8(gaussian(np.array(x) / 255.0,
...
def defocus_blur(x, img_size: int, severity=1):
    if img_size == 64:
        c = [(0.5, 0.6), (1, 0.1), (1.5, 0.1),
             (2.5, 0.01), (3, 0.1)][severity - 1]
    elif img_size <= 32:
        c = [(0.3, 0.4), (0.4, 0.5), (0.5, 0.6),
             (1, 0.2), (1.5, 0.1)][severity - 1]
    elif img_size >= 65:
...
def fog(x, img_size: int, severity=1):
    if img_size == 64:
        ...
        c = [(0.2, 3), (0.5, 3), (0.75, 2.5),
             (1, 2), (1.5, 1.75)][severity - 1]
    elif img_size >= 65:
...
def frost(x, img_size: int, severity=1):
    if img_size == 64:
        c = [(1, 0.3), (0.9, 0.4), (0.8, 0.45),
             (0.75, 0.5), (0.7, 0.55)][severity - 1]
    elif img_size <= 32:
        c = [(1, 0.2), (1, 0.3), (0.9, 0.4),
             (0.85, 0.4), (0.75, 0.45)][severity - 1]
    elif img_size >= 65:
        c = [(1, 0.4), (0.8, 0.6), (0.7, 0.7),
             (0.65, 0.7), (0.6, 0.75)][severity - 1]
    else:
...
def jpeg_compression(x, img_size: int, severity=1):
    if img_size == 64:
        ...
        raise ValueError(
            f"JPEG compression not defined for image size {img_size}")
...
def elastic_transform(x, img_size: int, severity=1):
    if img_size >= 32 and img_size <= 64:
        ...
        raise ValueError(
            f"Elastic transform not defined for image size {img_size}")
    ...
    return (
        np.clip(
            map_coordinates(image, indices, order=1,
...
class VisionCorruption:
    ...
    Args:
        ...
        severity (int): The severity of the corruption. Must be in [0, 4],
    """
    ...
    def __init__(
        self, img_size: Tuple[int, int, int], corruption_name: str, severity: int
    ) -> None:
...
def additive_gaussian_noise(x: np.ndarray, severity: int = 1, dataset_scale: float = 1) -> np.ndarray:
    ...
    Args:
        ...
        dataset_scale (float): The scale to scale the noise specific to the dataset.
    """
    c = TABULAR_ADDITIVE_SCALE[severity - 1] * dataset_scale
...
def multiplicative_gaussian_noise(x: np.ndarray, severity: int = 1, dataset_scale: float = 1) -> np.ndarray:
    ...
    """
    c = TABULAR_MULTIPLICATIVE_SCALE[severity - 1] * dataset_scale
    return x * np.random.normal(loc=1.0, scale=c, size=x.shape)


def additive_uniform_noise(x: np.ndarray, severity: int = 1, dataset_scale: float = 1) -> np.ndarray:
...
def multiplicative_uniform_noise(x: np.ndarray, severity: int = 1, dataset_scale: float = 1) -> np.ndarray:
    """Multiplies the input by a uniform noise with range [1-c, 1+c]."""
    c = TABULAR_MULTIPLICATIVE_SCALE[severity - 1] * dataset_scale
    return x * np.random.uniform(low=1.0 - c, high=1.0 + c, size=x.shape)


def multiplicative_bernoulli_noise(x: np.ndarray, severity: int = 1, dataset_scale: float = 1) -> np.ndarray:
...
class TabularCorruption:
    ...
    def __init__(self, corruption_name: str, severity: int, dataset_scale: float) -> None:
        self._corruption_name = corruption_name
        self._severity = severity
    ...
    def __call__(self, x: Union[np.ndarray, torch.Tensor]) -> torch.Tensor:
        ...
        x = self.corruption_dict[self._corruption_name](
            x=x, severity=self._severity + 1, dataset_scale=self._dataset_scale
        )
